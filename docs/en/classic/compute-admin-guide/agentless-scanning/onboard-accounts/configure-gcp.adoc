...
Refer to the GCP documentation to https://cloud.google.com/vpc/docs/shared-vpc[learn more about shared VPCs].[.task]

[#gcp-encrypted-volumes]
=== Scan Encrypted Volumes When Using Hub Mode

If your hub project is part of a GCP organization, and permissions were applied via organization onboarding, no further action is required.
This section only applies to single accounts that are onboarded separately.
