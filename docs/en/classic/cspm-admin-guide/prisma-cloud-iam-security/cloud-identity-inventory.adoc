...
On the Prisma Cloud console, access the Identity menu from the *Assets* side panel. Acesss the Assets side panel from the Alerts, Inventory or Investigate page. For example, On the Prisma Cloud console, select *Investigate* and run the following query to retrieve AWS cloud resources:
