...
On the Prisma Cloud console, you can access the Identity menu from the *Assets* side panel. Start, by selecting the Assets side panel from the Alerts, Inventory or Investigate page. For example, On the Prisma Cloud console, select *Investigate* and run the following query to retrieve AWS cloud resources:
