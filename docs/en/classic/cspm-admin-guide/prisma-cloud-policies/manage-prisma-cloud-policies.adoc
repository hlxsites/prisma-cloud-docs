...
[.procedure]
. To enable global settings for Prisma Cloud default policies click Settings and select *Enterprise Settings*.
+
...
====
* The audit logs include a record of all activities performed or initiated on Prisma Cloud. To view the audit logs click Settings and select *Audit Logs*.
