...
|*RLP-133444*
//Added post 24.2.2 when the issue was discovered. Move to Fixed Issues in 3.1 preserving the details of the Known Issue.

|The Asset Tag filter was incorrectly added to the Asset Inventory for all users of the Prisma Cloud console. This filtering capability is available only for participants of a Limited General Availability (LGA) program.
...
This issue will be resolved in an upcoming release, by surfacing the Asset Tag filter only on LGA enabled tenants.

// Add following to Fixed Issues post 3.1.

//Resolution:

//The UI team has implemented a fix to ensure the Asset Tag Filter option is only visible to LGA users. This prevents confusion and errors encountered by non-LGA users trying to utilize a non-functional feature.
