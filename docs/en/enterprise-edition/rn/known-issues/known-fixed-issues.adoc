...
|*RLP-133444*
//Added post 24.2.2 when the issues was discovered. Moved to Fixed Issues in 3.1 preserving the details of the Known Issue.

|The *Resource Tag Filter* option was incorrectly enabled for all users on the Prisma Cloud console. Ennablement should have been restricted to users who signed up for a Limited General Availability (LGA) trial.

As a Non-LGA user you may have experienced the following:

* Asset Tag Filter option available in the UI.
* Using the filter results in an error message.
