...
|Due to a newly introduced fail-safe mechanism that was aimed at preventing any adverse impact on customer traffic and avoiding downtime, the communication between a defender and the console gets interrupted and causes a temporary loss of coverage in some cases.
