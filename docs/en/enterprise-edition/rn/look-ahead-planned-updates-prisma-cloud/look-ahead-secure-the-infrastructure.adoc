...
Read this section to learn about what is planned in the 24.2.1 CSPM Platform, Agentless container host, Agentless Host Security, CIEM, Data Security, and CDEM releases. The Look Ahead announcements are for an upcoming release and it is not a cumulative list of all announcements.
...
All Prisma Cloud environments except app.cn and app.gov are updated to the Darwin release. Contact your Prisma Cloud Customer Success team for more details.
...
//* 24.1.2 (Jan 29-Feb 6) - app.cn -- moved to 24.2.1
...
//RLP-76433 - verify with PM moving blurb from LA to 24.2.1 RN
...
+
Because Prisma Cloud typically has 2 releases in a month, the release naming convention in GitHub is PCS-<year>.<month>.<release-chronology, 1 or 2>. For example, PCS-24.2.1.
...
Prisma Cloud will update the `gcloud-cloud-firestore-native-database` API to exclude the `earliestVersionTime` field from the resource configuration.
...
Prisma Cloud will update the `gcloud-compute-autoscaler` API to exclude the `recommendedSize` field from the resource configuration.
