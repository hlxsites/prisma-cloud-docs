...
To ensure continued connectivity to the Prisma Cloud console, review the following IP list and add the new addresses to your allow lists before February 2024. Failure to do so will result in tenant connectivity issues.

[NOTE]
====
Current (old) IPs will be deprecated once the migration is complete.
====
...
|Resource / Asset ID Field
|manifest-1701835039773
|https://www.googleapis.com/deploymentmanager/v2/projects/pcs-policy-vac/global/deployments/example-deployment-nc1/manifests/manifest-1701835039773

|Resource / Asset Name Field
|manifest-1701835039773
|example-deployment-nc1-manifest-1701835039773

|Ingestion Behaviour
|All Manifest resources that are part of a Deployment are ingested
|* Only the latest Manifest resource of a Deployment is ingested

* Older Manifest resources of a Deployment are deleted

[NOTE]
====
Only latest manifest is ingested as the manifest has both the initial configuration and the fully-evaluated configuration of the deployment.
====

|Alert Behaviour
|No OOB or Custom Policies are associated with this API
|If alerts are open for a custom policy associated with this API, then after this fix,

* Open Alerts are resolved as the previously ingested resources are deleted
* Post ingestion of the latest resources, alerts are open again for the recently ingested resources

|Resource Configuration
|CSP Response is ingested as is - No changes
|CSP Response is ingested as is  - No changes
