...
*Impact:* Resolves missed alerts where `0.0.0.0` is positioned in the middle of the comma-separated `ipRangeFilter` string rather than at the start or end.
...
*Impact:* Resolves false positive alerts for API Gateway methods configured with the OPTIONS method.
