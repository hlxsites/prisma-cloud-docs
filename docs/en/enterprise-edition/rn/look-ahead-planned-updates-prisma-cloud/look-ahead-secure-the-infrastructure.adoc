...
Impact:
Features that rely on historical audit data will be impacted:
...
* Alert Details: For alerts triggered by audit events, the associated event details will no longer be available for investigation if the *Investigate* action is clicked more than 30 days after the event occurred.
* Policy Evaluation: Evaluation of audit-event-based policies will focus on events within the new 30-day window.
* General Impact: Any other platform features or custom workflows that rely on the retention of audit events beyond 30 days may be impacted.
...
No additional action is required. The update will be rolled out automatically across all stacks. If your compliance or operational requirements necessitate audit log retention beyond 30 days, we recommend ensuring your CSP (AWS, Azure, GCP) is configured to archive these logs in their respective native storage (e.g., S3, GCS, or Azure Blob Storage).
...
|*Changes–* The policy RQL will be updated to cover Dataproc clusters using zonal GKE.
...
|*Changes–* The policy RQL will be updated to remove the `encryptionSettings` parameter.
...
You must manually add the permissions to a custom role.
