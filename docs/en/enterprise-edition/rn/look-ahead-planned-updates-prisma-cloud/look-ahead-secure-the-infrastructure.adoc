...
|Starting with the 5.1 release, Prisma Cloud will support the ingestion of VNet flowlogs along with the existing support for the ingestion of NSG logs for Microsoft Azure account onboarding. VNet flowlogs will also be added as a data source to existing OCI policies. There will be no impact to existing tenants and no additional action is needed. 
