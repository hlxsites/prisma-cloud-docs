...
//RLP-151640 - add 4 screenshots

|*Secrets* attributes within the *Investigate > Asset* search help you prioritize and find assets with secret exposure risks. Along with Findings and Vulnerablities, you can now also use Secrets as a security context to identify Attack Paths in your environment. You can create Custom Policies based on the asset query. You have to enable *Agentless Scanning* during onboarding in order to use *Secrets*.

image::secrets-2.png[]
//<autosuggest screenshot>

You can view your search results in both Graph and Table modes. In the Table mode, a new *View by: Secret* allows you to view all the secret detections that match the query. In the Graph mode, each Secrets node is a representation of all the secret detections of that specific asset. 
...
Click on the Secrets node and then click *View Details* to see additional information on the Secrets detected. 
