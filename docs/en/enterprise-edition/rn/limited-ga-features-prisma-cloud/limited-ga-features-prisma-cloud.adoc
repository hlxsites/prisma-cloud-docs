...
|*Secrets* attributes within *Investigate > Asset* search help you prioritize and find assets with secret exposure risks. Along with Findings and Vulnerablities, you can now also use Secrets as a security context to identify Attack Paths in your environment. You can create Custom Policies based on the asset query. You have to enable *Agentless Scanning* during onboarding in order to use *Secrets*.
...
In the Graph mode, each Secrets node is a representation of all the secret detections of that specific asset. 
