...
Along with the above enhancements, Prisma Cloud also includes *Tag-based Role-based Access Control* (RBAC) with Resource Lists that is built on Prisma Cloud’s existing RBAC capabilities, which allows your System Administrators to define limited access to cloud assets by introducing support for https://docs.prismacloud.io/en/enterprise-edition/assets/pdf/tag-based-rbac-resource-lists-lga.pdf[Tag-based Resource Lists for Roles].
