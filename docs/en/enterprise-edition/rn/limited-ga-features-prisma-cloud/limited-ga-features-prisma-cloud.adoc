...
|https://docs.prismacloud.io/en/enterprise-edition/assets/pdf/tag-based-rbac-resource-lists-lga.pdf[Tag-based Role-based access control (RBAC) with Resource Lists] builds upon Prisma Cloud’s existing RBAC capabilities that allow Administrators to define limited access to cloud assets by introducing support for Tag-based Resource Lists for Roles.
