...
|Prisma Cloud now includes the following enhancements:

* Time to Alert and Search on the Investigate page is three times faster. 
* Support for asset exclusion in `config from` network queries results.

The following enhancements are specific to AWS:

* Support for NLB and ALB listener port analysis. 
* Display East-West traffic over transit gateway. 
* Display East-West traffic within a single VPC.
