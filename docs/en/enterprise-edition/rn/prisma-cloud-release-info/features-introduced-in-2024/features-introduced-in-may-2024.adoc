...
|*Support for Agentless Scan in AWS Onboarding*
...
|*CNA Kubernetes (K8s) Container Exposure*
...
|You can now customize your https://docs.prismacloud.io/en/enterprise-edition/content-collections/application-security/risk-management/monitor-and-manage-code-build/fix-code-issues[pull request titles and branch names] for suggested fixes. This feature enables organizations to enforce a consistent naming convention, ensuring clear and contextual information about the purpose of the fix and makes it easier for team members to understand the changes being made.
...
//check with engg RLP-132803, RLP-139583, and RLP-136776
...
|Identifies Amazon OpenSearch Service domains that have service software updates available but not installed for the domain.
...
|Identifies Amazon Neptune DB clusters lacking sufficient backup retention tenure. 
...
|Identifies Amazon Neptune DB clusters where audit logging is disabled or audit logs are not published to Amazon CloudWatch Logs.
...
|Identifies Amazon DocumentDB cluster where audit logging is disabled or audit logs are not published to Amazon CloudWatch Logs. 
...
|Identifies the AWS Network Firewall for which delete protection is disabled. AWS Network Firewall manages inbound and outbound traffic for the AWS resources within Virtual Private Clouds (VPCs). 
...
|Identifies AWS Application Load Balancers that are not configured to drop HTTP headers. 
...
|Identifies Azure Storage accounts that are enabled with cross tenant replication. 
...
|Identifies Azure App services that have HTTP logging disabled. 
...
|Identifies Azure App Service Environments that are configured with weak TLS Cipher suites.
...
As a security best practice, it is recommended to grant the least privileged access, such as granting only the permissions required to perform a task, instead of providing excessive permissions.
...
|The following APIs are now deprecated because Azure had announced the retirement of Azure Data Lake Analytics and Azure Data Lake Storage Gen1 Services. Due to this, Prisma Cloud will no longer ingest metadata for the following APIs:
...
*Impact*—If you have a saved search or custom policies based on this API, you must delete those manually. The policy alerts are resolved as Policy_Deleted.
