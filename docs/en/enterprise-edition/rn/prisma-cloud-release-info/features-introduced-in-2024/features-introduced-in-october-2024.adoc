...
Granting a new identity permissions that belong to a different role, to edit and run cloud resources may lead to a scenario where multiple roles and trust relationships lead to privilege escalations. https://docs.prismacloud.io/en/enterprise-edition/content-collections/search-and-investigate/permissions-queries/permissions-query-attributes[Role chaining detection] allows you to better manage complex permissions configurations.
