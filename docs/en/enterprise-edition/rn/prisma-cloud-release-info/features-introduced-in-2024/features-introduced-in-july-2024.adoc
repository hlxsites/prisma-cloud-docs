...
|Enhancements to Prisma Cloud's CIEM capabilities enable you to extract relevant information about GCP service accounts with access to your AWS environment by assuming IAM roles to defend against cross-cloud lateral threat vectors proactively. Use the RQL query below to investigate trust policies to handle identities and permissions across your multi-cloud environments securely
...
|Prisma Cloud's IAM functionality now includes the ability to  querying when IAM roles were last used to detect unused roles. Available for AWS, Azure, and GCP, this RQL clause allows you to determine the days passed since the last login or use of a role. Inactivity will also be listed on the IAM Role asset page. Add the RQL clause below to queries to find days passed since last login info for roles, service principals and service accounts.
...
|Cloud Application Security’s (CAS) detection of CVE vulnerabilities within your runtime has been enhanced. You can now leverage the “Vulnerability Risk Factors” filter on the “Projects” page to identify vulnerabilities based on “Package in Runtime,” “Repository in Runtime,” and “Manifest in Runtime” factors.  This allows you to focus and mitigate vulnerabilities at their source, preventing them from reaching runtime environments.
For more information refer to https://docs.prismacloud.io/en/enterprise-edition/content-collections/application-secu[…]software-composition-analysis/software-composition-analysis.

|*Added support for .NET CPM*
...
|Added support for NuGet’s central package management (CPM), which allows you to manage common dependencies for many different projects in a single location, the  Directory.Packages.props file. This enhancement is part of our Software Composition Analysis (SCA) capabilities.
Link: https://docs.prismacloud.io/en/enterprise-edition/content-collections/application-security/supported-technologies
