...
|RLP-135896 - *Compliance trend*

|tt:[Update] *Implicit End Timestamp for V2 Config Search Endpoints*

tt:[*24.4.1*]
//RLP-136227

|The `Time Range` model used in V2 Config Search endpoints Requests `search/api/v2/config` and `search/api/v2/config/:id` requires a `startTime` value only. The `endTime` is implicitly set to the current system time. 
