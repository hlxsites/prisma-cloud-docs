...
//Verify and add: RLP-152289, RLP-151629, RLP-151431, RLP-151095, RLP-149870
...
|Action Plans utilizes AI to deliver actionable remediation steps that are tailored to maximize risk reduction while requiring you to take minimal actions, thereby enhancing efficiency and efficacy in securing assets. 
//provides you with tailored recommendations for remediating security issues across your cloud environments and codebases with minimal user action.

* *Prioritization*: Action Plans prioritize based on security context and Prisma Cloud alerts. Action Plans group together alerts and assets that can be secured through a single fix, ensuring your security team spends time on the most effective outcomes.

* *Execution and Delegation*: Action Plans leverage your integrations on Prisma Cloud to help delegate security fixes to your team through Jira yickets and/or Slack messages with a single click.

* *Detailed Visibility*: Action Plans ensure that every alert resolved or asset impacted is visible in one location, ensuring detailed context to your security teams.

* *Security Fix Efficiency*: Using machine learning and generative models, Action Plans help summarize tasks across multiple alerts impacting the same asset, ensuring that there is a comprehensive plan to reduce alerts within the least number of steps.

//<add image/gif and link to document>
...
|*Placeholder text for RLP-150693*
//RLP-150693, RLP-150692, RLP-150691
//Highest CVE Score and Severity in the CBDR graph Vulnerability Preview Card

|The *Most Important Vulnerabilities* widget now displays the highest *CVE Score* and *Severity* associated with the CVE ID under the CVE preview card.
...
|*Placeholder text for RLP-145638*
//RLP-145638

|Pseudo assets, such as regions and accounts are excluded from Alert reports.
...
* Use the new API to programatically leverage the new pagination and filter capabilities to streamline your use cases. 
...
|The policy that flags Azure DNS zones with dangling DNS records is updated. This change prevents false positives for stopped resources and ensure only genuine vulnerabilities are flagged.
...
*Alerts Impact—* Alerts will be reduced as existing false positives will be resolved as `Policy Updated`.
...
|The Remediation Steps for the policy have been updated.
