...
|*AWS Cognito service role does not have Cognito identity pool verification*

tt:[*24.3.2*]

//RLP-132945

|Identifies an issue with the trust policy of the AWS Cognito service role, as it lacks a condition restricting access to the current identity pool. The 'aud' claim in user pool authentication contains the client_id requested by the user. The absence of the 'aud' claim in the service role's configuration allows users with Cognito-signed tokens to request access across user pools without restriction. This oversight increases the risk of unauthorized access, compromising access controls and elevating the potential for data breaches within the AWS environment. To address this concern and bolster security measures, it is advisable to implement proper validation of the 'aud' claim.
...
|Identifies the AWS Cognito service role has wide privileges and does not validate that the user is authenticated using the 'amr' field. This oversight allows anonymous users with valid tokens to assume the role, potentially exploiting its broad privileges and disrupting workflows. To mitigate this, it's crucial to limit the AWS policies to not have a '*' wildcard in the action and resource section of the policy statement and roles. Ensuring limiting privileges according to business requirements can help in restricting unauthorized access and misuse of resources.
...
|*AWS Redshift cluster instance configured with public accessibility and using a commonly used master username*

tt:[*24.3.2*]

//RLP-132006

|Identifies AWS Redshift clusters with the 'publicly accessible' setting enabled and master usernames set to commonly used options like 'awsuser', 'administrator', or 'admin'. Making Amazon Redshift clusters publicly accessible heightens the risk of unauthorized access and Distributed Denial of Service (DDoS) attacks. Using standard usernames increases the vulnerability to brute-force password attacks by potential intruders. As a recommended security measure, it is advised to disable the 'publicly accessible' parameter for Redshift clusters. Furthermore, to reduce the possibility of authentication-based attacks, it is suggested to customize the master username.
...
|Identifies s3 buckets that have CloudTrail logs using a Customer Managed Key (CMK) that has been scheduled for deletion. Deleting keys in AWS KMS is destructive and potentially dangerous and might be related to a defense evasion technique used by adversaries. After a key is deleted, you can no longer decrypt the data that was encrypted under that key, which impacts the CloudTrail logs and causes them to become unrecoverable. It is recommended to regularly monitor the key used for encryption should not be accidentally deleted.
...
|*AWS SNS Topic not encrypted by Key Management Service (KMS) using a Customer Managed Key (CMK)*

tt:[*24.3.2*]

//RLP-133587

|Identifies AWS SNS Topics that are not encrypted by Key Management Service (KMS) using a Customer Managed Key (CMK). It is a best practice to use customer managed KMS Keys to encrypt your SNS Topics. It gives you full control over the encrypted data by protecting the contents of topic messages.
...
|Identifies AWS Default VPCs that are being used. A default VPC is created for your AWS account when you create an AWS account. It includes a default security group and a default network access control list (NACL). Default VPCs have some limitations that might not be suitable for all use cases. As best practice, it is recommended to use a custom VPC configuration based on your security and networking requirements instead of using the default VPC, so that you can have full control over the VPC network.
...
|Identifies AWS EKS clusters that does not have secrets encryption enabled. Using secrets encryption for your Amazon EKS cluster allows you to deploy a defense in depth strategy for Kubernetes applications by encrypting Kubernetes secrets with a AWS KMS key that you define and manage. It is recommended to  enable secrets encryption on your EKS clusters.
...
|Identifies Postgres RDS clusters with query logging disabled. The queries executed on RDS must be logged for monitoring and troubleshooting. To obtain adequate logs, an RDS cluster should have a non-default parameter group and two parameters configured: log_statement and log_min_duration_statement.
...
|*AWS VM instance in running state that is internet reachable with unrestricted access (0.0.0.0/0)*

tt:[*24.3.2*]

//RLP-124917
|*Changes—* The policy name is updated.
//The Attack Path policy that has this CNA policy and needs to be updated is: Unauthorized access risk due to a publicly exposed and vulnerable AWS EC2 instance

*Current Policy Name—* AWS VM instance in running state that is internet reachable with unrestricted access (0.0.0.0/0)

*Updated Policy Name—* AWS EC2 instance that is internet reachable with unrestricted access (0.0.0.0/0)
...
|*Updates related to new features introduced in CDEM*
...
|*RBAC updated for two APIs for Widgets*
...
|The following two APIs that were previously accessible only to System Admins are now accessible to roles that have the `Alerts_READ` permission:
