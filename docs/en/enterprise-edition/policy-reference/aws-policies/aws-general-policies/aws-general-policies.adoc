...
|xref:bc-aws-373.adoc[Bedrock Agent not encrypted with Customer Master Key (CMK)]
...
|xref:bc-aws-384.adoc[Hard-coded secrets found in Parameter Store values]
...
|xref:bc-aws-387.adoc[AWS SQS queue access policy is overly permissive]
| https://github.com/bridgecrewio/checkov/blob/main/checkov/terraform/checks/resource/aws/SQSOverlyPermissive.py[CKV_AWS_387]
|INFO

|xref:bc-aws-2-73.adoc[AWS SQS queue encryption using default KMS key instead of CMK]
