...
The above code sets 'public_access_prevention' to 'enforced' for the Google Cloud Storage bucket. This enables public access prevention, so the Google Cloud Storage bucket cannot be accessed publicly, and thereby resolves the issue identified in the policy 'CKV_GCP_114'.
The `bucket_policy_only` field is deprecated in favour of `uniform_bucket_level_access`which provides the same functionality. Both fields commonly enable or disable requesting object access control lists (ACLs).
Also, `uniform_bucket_level_access` provides a uniform permission structure that applies to all requests and is intended to replace the had-hoc binding of permissions at either the bucket or object level.
