...
If you want to create custom policies for build-time checks, see xref:../content-collections/governance/custom-build-policies/custom-build-policies.adoc[custom build policies].
...
On the Prisma Cloud management console, if you want to find all policies available for the build phase set the search filter for *Policy Sub Type* to *Build*. See xref:../content-collections/governance/manage-prisma-cloud-policies.adoc[manage policies] for details.