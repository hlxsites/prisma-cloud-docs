...
== The 'alpha cluster' feature is disabled on GCP Kubernetes engine clusters.
...
The policy identified by 'CKV2_GCP_19' is checking to ensure that the 'alpha cluster' feature is disabled in Google Cloud Platform's (GCP) Kubernetes engine clusters. This is important because enabling the alpha cluster feature can present several potential issues.
