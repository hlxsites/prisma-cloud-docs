...
Here are few frequently asked questions (FAQs) regarding Prisma Copilot.

* *What language models are used by Prisma Copilot?*
+
The Copilot uses the large language models (LLMs) provided by one of the leading LLM providers. Prisma Cloud has an enterprise level agreement with the LLM provider ensuring that no data is used to further train those LLMs.

* *Does Prisma Copilot use any of my data for model training?*
+
The Copilot *does not* use your data for training the models used in the Copilot. Prisma Cloud uses foundational models (LLMs trained with publicly available data) for two purposes: 
+
...
* *How does Prisma Copilot use my data to answer the questions it receives?*
+
Here's how the Copilot works and how it uses your data: 
+
** *Step 1 (question contextualization)*
+
When you type a question in Copilot, the question is analyzed by the Copilot to determine if additional data is required to augment the question. 
+
...
** *Step 3 (local query)*
+
The Copilot takes the response from the LLM and checks for accuracy. If the response is correct, the Copilot uses it to collect your data that is required to provide the final answer. 
+
...
** *Step 4 (final answer)*
+
The answer from the API endpoint is received by the Copilot, which is displayed as a graph. 
+
