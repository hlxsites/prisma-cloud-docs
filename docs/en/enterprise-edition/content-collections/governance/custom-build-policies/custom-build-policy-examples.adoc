...
The following examples demonstrate how to create a custom build policy for detecting secrets.

//* <<ss-basic-policy-definition, Basic Policy Definition>>

[#ss-basic-policy-definition]
==== Example #1 Basic Policy Definition
Use the sample guideline to create a basic policy definition using a single attribute. Multiple signatures, as shown in this example, will be treated as OR statements, meaning that all strings matching either pattern will be flagged.
...
==== Example #2 Detect Generic Passwords

This policy detects instances where literal strings starting with "password", "pass" or "pwd" are used directly in the code or configuration files. This is a security risk because it exposes the actual password value.

----
metadata:
  guidelines: "Find passwords starting with password|pass|pwd and the value format is not {var}.{var}"
  category: "secrets"
definition: 
  cond_type: "secrets"
  value:
