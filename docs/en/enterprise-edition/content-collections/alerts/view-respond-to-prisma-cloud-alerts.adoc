...
When you add a cloud account on Prisma Cloud and then delete it, you can no longer view alerts associated with that account on *Alerts > Overview* and the alert count does not include alerts for a deleted cloud account. After 24 hours, the alerts are permanently deleted.
