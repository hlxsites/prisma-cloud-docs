...
You can take action and manage alerts in a few different ways on Prisma Cloud. 
...
// Investigate from alert ID side panel

* Send to Jira

* View in Console

* View Config - View a snippet of the asset configuration in a  JSON format. This view enables you to 

* View Details - Takes you to Runtime Security > Monitor > Vulnerabilities

Some additional options that you can access when you select an asset in an alert and view  the Asset side panel are:
...
Suggest Least Privilege Access - When  an asset has an identity attached to it, for example an IAM role that grants access to an EC2 instance, on *IAM Details* you can configure least privilege access. You can define a time when permission of the asset will be considered as over permissive and get our suggestion for new configuration with only used (by all identities attached to the asset) permissions or existing configuration with the minimum excessive permissions.
