...
[.procedure]
. xref:../configure-external-integrations-on-prisma-cloud/integrate-prisma-cloud-with-cortex-xsoar.adoc[Integrate Prisma Cloud with Cortex XSOAR].

. Select "Alerts > Alert Rules" and either xref:create-an-alert-rule-cloud-infrastructure.adoc[Create an Alert Rule for Run-Time Checks] or select an existing rule to edit.
