...
[IMPORTANT]
====
When you add an alert rule policy filter, keep in mind that filters follow *AND* logic not OR. For instance, if you create an alert rule by selecting _policy label='Prisma Cloud'_ with _Compliance standard='CIS v2'_, an AND rule is applied. Scans will return results with policies that have both the Prisma Cloud label and the CIS v2 compliance standard.
====
+
[NOTE]
====
