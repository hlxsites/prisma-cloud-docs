...
To configure WAAS agentless with AWS VPC traffic mirroring, follow these steps.

[.procedure]

. Apply the permission template

.. Use the permission template ending in https://redlock-public.s3.amazonaws.com/waas/aws/agentless_app_firewall_permissions_template.yaml[agentless_app_firewall_permissions_template.yaml] and apply it to your to AWS CloudFormation console for your account. This template grants the necessary WAAS agentless permissions.

. Edit the template.

.. Edit the template by replacing the `"USER-agentless-app-firewall"` value with the actual username that you want to grant these permissions.
+
This AWS CloudFormation permission template is integrated with Amazon EC2 Auto Scaling, allowing seamless scaling of your WAAS agentless setup.
+
After applying the CloudFormation template, WAAS agentless permissions policies will be created in AWS as part of the `aws agentless permissions` stack.
+
WAAS agentless permissions policies created in AWS:
+
image::runtime-security/aws-agentless-permissions-stack.png[]

.. Deploy the WAAS agentless resources stack in AWS using the `aws agentless setup` stack. This will set up the necessary resources for WAAS agentless operation.
+
WAAS agentless resources stack deployed in AWS:
+
image::runtime-security/aws-agentless-setup-stack.png[]

. Optionally,onboarding AWS Accounts with Prisma Cloud

.. If you onboard your AWS accounts using Prisma Cloud, you can add the necessary WAAS agentless permissions to the same role used by Prisma Cloud. During the setup, provide the Prisma Cloud role as a parameter in the CFT template.
+
After successfully deploying the CloudFormation stack, new policies will be created and attached to the selected role, ensuring the role has the necessary permissions for WAAS agentless functionality. For detailed instructions on how to apply cloud formation templates, refer to the https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cfn-console-create-stack.html[AWS documentation].

. Creating an AWS Cloud Account

* Create an AWS Cloud account under *Compute > Manage > Cloud accounts* using the `Access key` from your AWS account. Ensure that Cloud discovery is enabled to discover the EC2 instances to mirror.
