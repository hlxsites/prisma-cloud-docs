...
. Click on the workload in the radar and open xref:#connectivity_monitor[`WAAS connectivity monitor`] by clicking on the WAAS tab.
...
|Click on `View recent errors` in the xref:#connectivity_monitor[`WAAS connectivity monitor`] to view errors.
