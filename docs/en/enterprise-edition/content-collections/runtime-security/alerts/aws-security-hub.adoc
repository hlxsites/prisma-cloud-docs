...
+
image::runtime-security/add-aws-sqs-integration.png[width=350]
...
. Go to *Runtime Security > Manage > Alerts > Add profile*.
...
. Click *Save*.
+
image::runtime-security/alert-profile-aws-sqs.png[width=350]
+
This alert will be triggered based on your runtime policy settings under *Runtime Security > Defend* policy settings, and the JSON payload message will be sent to the AWS SQS queue.
+
image::runtime-security/alert-message-in-aws-sqs.png[width=300]
+
image::runtime-security/alert-message-awssqs-json-body.png[]
+
*Limitations*
+
