...
* ​Cloud Native Network Segmentation (CNNS) policies for both containers and hosts have been deprecated. It should be used solely for radar visibility and only in scenarios where alternative network monitoring modes are unavailable. The current recommended configuration is to disable all enforcement related to CNNS-based network monitoring.
