...
Prisma Cloud Compute uses the Go programming language cryptographic libraries to protect all network communications via the Transport Layer Security (TLS) v1.3 protocol.

=== Prisma Cloud Compute Self-Hosted

The User Interface (UI) and API access is protected using server side TLS v1.3 authentication.
...
The Console and Defenders communication using a mutual TLS v1.3 web-socket session (default TCP 8084).
...
The certificate/keys used for the Console to Defender mutual TLS v1.3 web socket session are generated during the Console's initiation process.
