...
*Radar > Cloud* shows the serverless functions as undefended until the functions are not scanned for vulnerabilities, regardless of auto defend status.
Add a *Defend > Vulnerabilities > Functions > Functions* rule to trigger a scan on the relevant functions. The Radar now shows the previously undefended functions as defended.
