...
* Increase the Pod limit to 4 GB when activating WAAS OOB or in-line on K8s cluster. This resolves out-of-memory (OOM) issues when running an inline container firewall on Defender nodes operating within Kubernetes (k8s), particularly when there are more than 10 protected workloads on the same node.
