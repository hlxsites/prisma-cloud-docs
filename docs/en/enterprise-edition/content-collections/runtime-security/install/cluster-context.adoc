...
NOTE: For Azure AKS, cluster name detection is not supported for clusters deployed within custom resource groups. The defender's Azure-specific cluster name resolution relies on the default name of the resource groups that contain that cluster name, for example, `MC_myResourceGroup_myAKSCluster_eastus`, see https://learn.microsoft.com/en-us/azure/aks/faq#why-are-two-resource-groups-created-with-aks-[Microsoft documentation] for more details. The fallback is to use the server extracted from the kubeconfig as the cluster name, e.g `https://10.2.202.4:443`.
