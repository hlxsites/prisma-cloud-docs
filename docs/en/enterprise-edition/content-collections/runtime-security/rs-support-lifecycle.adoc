...
The numbering format of releases is <Major_version>.<Minor_version>.<Build_number>. For example, the current release is `34.00.141`, which has the internal name `Quinn`. The next minor release will be `34.01.build_number`.
...
=== Extended Support lifecycle for Prisma Cloud Compute Edition and modules
...
Starting from Prisma Cloud Compute Edition release 21.xx ("Iverson"), which shipped in the second half of calendar year 2021, Prisma Cloud has extended the support lifecycle as explained in the following sections. 
