...
Agentless scanning is triggered only for new cloud accounts onboarded with the agentless scanning option enabled. If accounts are onboarded with this option disabled, scanning is not triggered. 

Similarly, when agentless scanning is enabled for existing accounts, scanning is not triggered immediately. By default, agentless scans are triggered every 24 hours.

See Modifying the Agentless Scan Cycle Interval

You can change the configuration after onboarding your cloud accounts in *Runtime Security > Manage > Cloud accounts*.

See Editing Agentless Scan Settings for an Account in Runtime Security.
