...
E.g., assuming the max scanners limit is set to 50 scanners, so the limit is per region being scanned. Assume 2 accounts scanned, with 1 region each - agentless can scale across 2*1*50 = 100 scanners in total.
|Scales only within the hub account, overall scan duration is longer. 
This is because agentless can only scale across the max scanners defined on the hub account, regardless of amounts of accounts/regions scanned.
Note: In addition, encrypted volumes in AWS are required to be copied, so scaling in hub mode will be bottle-necked by the concurrent snapshot copy limit in AWS (which is 20 by default): https://aws.amazon.com/about-aws/whats-new/2020/04/amazon-ebs-increases-concurrent-snapshot-copy-limits-to-20-snapshots-per-destination-region/
