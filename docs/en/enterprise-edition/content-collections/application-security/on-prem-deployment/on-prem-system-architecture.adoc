...
The following services are deployed and run in the client's Kubernetes (K8s) cluster. Connectivity between the client and Prisma Cloud is facilitated through Transporter, allowing Prisma Cloud to send requests to the client to initiate tasks. All the images mentioned below are pulled from the Prisma Cloud AWS Elastic Container registry (ECR).
...
The customer's K8s pulls images from the Prisma Cloud ECR using the 'Pull Image Secret', a token that is refreshed automatically every 12 hours.
...
NOTE: These methods focus solely on establishing the connection between your VCS and Prisma Cloud. Other Prisma Cloud components are not directly involved in the VCS to Prisma Cloud Server communication channel itself.