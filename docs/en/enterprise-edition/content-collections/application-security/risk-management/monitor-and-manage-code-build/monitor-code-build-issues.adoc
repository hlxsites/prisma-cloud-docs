...
* *Validity*: Leverages public APIs to determine if a secret is Valid and should be prioritized, Invalid and can be deprioritized, or Unknown if Prisma Cloud is unable to determine the validity.
* *Privileged*: Determines if the AWS Access Key that is exposed has privileged permissions based on our IAM Security capabilities.
* *Found in History*: If the secret no longer exists in the current commit, but was found in history scanning.
* *IaC Resource*: Identifies if a secret was found inside an IaC resource block.
