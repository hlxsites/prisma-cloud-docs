...
Yor trace tags are accessible for you on the Prisma Cloud console, where you can choose to enable or disable them. Yor scan runs for every resource, and if any resource is not compliant with a tag rule, Yor automatically creates a PR (pull request) for the repository with the appropriate tags per resource. You can access your version control system to merge the tag PR and add the tags to the resource. Note that only one tag PR can be open per repository. If there is already an open tag PR, Prisma Cloud will not create another.
