...
// to do For more information on default License policies refer to (create License policies and link)
...
+
Refer to the xref:../../../administration/create-custom-permission-groups.adoc[Create Custom Permission Groups] for more information.

. Select the required permissions in the *Policies* menu under the *Assign Permission* step > complete the Group Permission setup.
+
image:[]
...
.. Filter *Predefined policies* by policy type or specific license category.
+
* *Filter by Policy type*: Under *Policy Label*, select *License Policy*. All predefined policies related to licenses, that is all weak copyleft, strong copyleft, and non-permissive license policies, are displayed
* *Filter by specific License category*: Under *Policy Label*, select a license category: *Weak copyleft*, *Strong copyleft*, or *Non-permissive license*. All predefined policies related to the selected category are displayed
...
Enforce license violations by severity and policy labels, ensuring developers are promptly notified during pull requests if they introduce a package with an unapproved license. For further details on Enforcement rules, refer to xref:../../risk-management/monitor-and-manage-code-build/enforcement.adoc[Enforcement].
...
.. Select a license policy, including both default and custom policies. 
+
