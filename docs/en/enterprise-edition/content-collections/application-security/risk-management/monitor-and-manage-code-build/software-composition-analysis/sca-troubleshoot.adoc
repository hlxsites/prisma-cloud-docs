...
. Select the icon to display the list of detected errors, and the number of files in which the error was detected.
+
xref::application-security/sca-vulnerabilities1.1.png[]

. Selecting a finding displays expanded details including details of the repository in which the issue was detected, and the path to the issue.
+
