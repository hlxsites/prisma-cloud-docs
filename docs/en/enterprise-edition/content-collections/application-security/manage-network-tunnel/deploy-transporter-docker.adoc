...
.. https://docs.docker.com/engine/install/[Install Docker]. The Transporter Client requires Docker to be installed in your environment with network access to your self-hosted provider, such as your VCS or Terraform Enterprise (Run Tasks). 
...
+
NOTE: Only after the Transporter has run successfully can Prisma Cloud authenticate and establish a communication channel with your provider (such as VCS/Terraform Enterprise). Once the connection is established, Transporter will use WebSockets to communicate with each other.
...
+
NOTE: To delete the Transporter, you need to first remove the existing VCS/Terraform Enterprise integrations associated with it.
