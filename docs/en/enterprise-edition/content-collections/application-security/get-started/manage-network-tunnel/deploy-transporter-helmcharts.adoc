...
Setting up the Transporter network tunnel using Helm charts provides automated and efficient configuration management. Transporter can be deployed  in a new or existing cluster environment.
...
+
As displayed, you can encrypt traffic at either the pod or ingress level. You can omit encryption when dealing with both ingress and pod. You can also decide to interact directly with the service and pod without involving ingress.
...
+
Example: Set the service to a different type (e.g. LoadBalancer) and port (e.g. 8000), and change the service section’s type and port values:
+
[source,yml ]
---
service:
