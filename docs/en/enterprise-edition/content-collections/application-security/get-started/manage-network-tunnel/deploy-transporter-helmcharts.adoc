...
. Pull the Docker image: Copy the command from the *Docker pull CLI command* field and run it in your CLI terminal.
...
+
Run the following commands with the Transporter environment variables that were generated during the installation procedure above in the folder where you extracted the Helm chart ZIP file:
+
[source,yml]
---
