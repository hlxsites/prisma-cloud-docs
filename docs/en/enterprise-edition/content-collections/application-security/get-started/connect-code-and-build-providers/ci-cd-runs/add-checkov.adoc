...
* Ensure that the *--repo-id* parameter includes the `<custom_text>/<repo_name>` structure, with the `<repo_name>` matching the exact name of the directory.
+
Example: Suppose the present working directory is `Users/<custom_text>/<repo_name>`. In this case, the *--repo-id* parameter must be: `--repo-id <custom_text>/<repo_name>`, ensuring that *<repo_name>* precisely matches the directory name within the structure.
