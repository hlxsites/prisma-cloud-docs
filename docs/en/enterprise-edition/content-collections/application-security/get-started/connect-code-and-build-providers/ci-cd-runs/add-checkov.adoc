...
In order to use Checkov with a proxy, set the `HTTP_PROXY` and `HTTPS_PROXY` environment variables (some environments will use lowercase `http_proxy` and `https_proxy`). Example:

[source.shell]
----
# Set proxy environment variables
export HTTP_PROXY=https://proxy.domain.site  # Alternatively use http_proxy for environments that require it
export HTTPS_PROXY=https://proxy.domain.site  # Alternatively use https_proxy for environments that require it
