...
* Execute Checkov in the present working directory. Example: `Users/xxx/Downloads/terragoat-master`. It is recommended to use the absolute file path.
* The `--repo-id` parameter  must match the exact name of your current working directory precisely. Example: `--repo-id is <x>/terragoat-master` where <X> can be any input  
