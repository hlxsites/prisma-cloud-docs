...
.. Best Practice (Mandatory for SCA vulnerability suppression): To suppress vulnerabilities identified by SCA scans, it's mandatory to run Checkov within your current working directory (<current_directory_path>). The --repo-id parameter must  match the name of this directory exactly.

* Execute Checkov in the present working directory. Example: `Users/xxx/Downloads/<current directory>`. It is recommended to use the absolute file path.
* The `--repo-id` parameter must match the exact name of your current working directory precisely: `--repo-id <current directory>`.   
