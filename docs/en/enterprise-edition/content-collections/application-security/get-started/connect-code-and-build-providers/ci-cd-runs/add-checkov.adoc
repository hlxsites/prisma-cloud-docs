...
NOTE: Using `--policy-metadata-filter` and `--policy-metadata-filter-exception` will not work with the Developer role and will require one of the other roles or a custom permission group with the above permissions
