...
In order to use Checkov with a proxy, set the `HTTP_PROXY` and `HTTPS_PROXY` environment variables. Some environments will use lowercase `http_proxy` and `https_proxy`, so we recommend setting all four. Example:
