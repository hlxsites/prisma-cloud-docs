...
* On Prisma Cloud Open the wizard: In *Application Security*, select *Home* > *Settings* > *Connect Provider* > *Code & Build Providers* > *AWS CodeCommit*.
+
...
* On AWS, open Cloudformation> Stacks. Verify in the list that that the integration is displayed with a success status.
