...
. Before you begin.
.. Grant the Prisma user integrating Prisma Cloud with Bitbucket user permissions as specified in  <<#user-permissions, User Permissions>> below.
+
...
* *CI/CD module permissions*: An optional phase has been added for creating a PAT (Personal Access Token) with permissions tailored for CI/CD operations. These permissions include *read* access to repositories and *administrative* capabilities

For more information on Bitbucket permissions refer to https://developer.atlassian.com/cloud/bitbucket/rest/intro/#authentication[Authentication methods].
