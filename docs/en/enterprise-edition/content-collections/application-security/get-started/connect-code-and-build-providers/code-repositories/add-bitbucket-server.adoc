...
Prisma Cloud supports multiple integrations with Bitbucket Data Center/Server instances. Once you've initially integrated with Prisma Cloud, you can add more integrations using either the existing user or team token, if the current server has additional repositories requiring integration. Alternatively, if you're integrating with a new server, you can generate a dedicated token for the process. 
