...
*IaC* issues: A red mark beside a line number indicates issues within a resource. Select the mark to view the first issue, including its name, severity, and affected lines of code. If applicable, line(s) of code displaying the fix are shown, illustrating the correction, or a code diff highlighting the changes between the error and the fix may be provided. If there are multiple issues, the total count is displayed, and an arrow enables scrolling through them. In addition, remediation options are displayed. To display a detailed view of the issue in the Problems Tool, click *Console*.   
...
*Secrets*: A red mark next to a line number indicates a secret in the file, including severity. Remediation options are displayed. To display a detailed view of the issue in the Problems Tool, click *Console*.
