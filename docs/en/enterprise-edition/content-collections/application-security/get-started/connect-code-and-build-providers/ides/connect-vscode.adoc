...
. Access the Marketplace: Navigate to the https://marketplace.visualstudio.com/items?itemName=PrismaCloud.prisma-cloud[Prisma Cloud extension on the Visual Studio Marketplace] OR:
+
On the Prisma Cloud console: Select *Settings* > *Connect Provider* > *Code & Build Providers* > *VScode* (under IDEs). You are redirected to the Marketplace > Search for *Prisma Cloud* > select the Prisma Cloud plugin.
