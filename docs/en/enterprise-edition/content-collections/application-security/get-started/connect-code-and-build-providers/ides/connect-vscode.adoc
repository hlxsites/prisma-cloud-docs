...
Install the plugin directly from the VS Code IDE or through JetBrains Marketplace.  

==== Install directly from VS Code IDE

. In VS Code, open the *Extensions* panel > search for *Prisma Cloud* > *Install*.
+
NOTE: Restart your IDE if prompted.

==== Download and install the plugin from the Visual Studio Marketplace

. Access the Marketplace directly: Navigate to the https://marketplace.visualstudio.com/items?itemName=PrismaCloud.prisma-cloud[Prisma Cloud extension on the Visual Studio Marketplace]
+
OR:
+
Access the Marketplace from the Prisma Cloud console: Select *Settings* > *Connect Provider* > *Code & Build Providers* > *VScode* (under IDEs). You are redirected to the Marketplace > Search for *Prisma Cloud* > select the Prisma Cloud plugin.

. In the Marketplace, select *Install* > *Open Visual Studio Code.app*.
...
* *Problems Tool*: Provides detailed information, including available remediation options, for findings selected in the navigation bar, or through the *Console* option in the Code editor
