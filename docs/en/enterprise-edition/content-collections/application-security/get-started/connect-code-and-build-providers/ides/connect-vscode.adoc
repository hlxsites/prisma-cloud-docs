...
. Access the Marketplace: Navigate to the https://marketplace.visualstudio.com/items?itemName=PrismaCloud.prisma-cloud[Visual Studio Marketplace] > search for *Prisma Cloud* > select the Prisma Cloud plugin OR:
+
On the Prisma Cloud console: Select *Settings* > *Connect Provider* > *Code & Build Providers* > *VScode* (under IDEs). You are redirected to the Marketplace.
. In the Marketplace, search for *Prisma Cloud* > select the Prisma Cloud plugin > *Install* > *Open Visual Studio Code.app*.
