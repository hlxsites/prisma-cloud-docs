...
* *Specific Frameworks*: Scan specific frameworks such as `arm json secrets serverless`. You can add multiple frameworks using spaces between the values in the command. Refer to the xref:../../cli/cli-cmnd-ref.adoch[Checkov CLI Command Reference] for more information
