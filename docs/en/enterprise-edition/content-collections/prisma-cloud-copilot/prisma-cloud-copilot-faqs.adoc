...
+
The ephemeral data processing functionality of Prisma Cloud Copilot is provided by a LLM vendor in the location specified in the below table. Data is processed in memory by the LLM vendor for no longer than necessary to service the specific request in real-time and not retained in such a location. 
+
The user prompt (what a Prisma Cloud user types in the Copilot) is sent to the LLM, which is hosted in a data center located as per the table below. The LLM translates the user prompt into an API request or an RQL query, which is sent back to the Copilot. Either response (API request or RQL query) is executed locally, within a Prisma Cloud stack and without LLM involvement.
+
[cols="50%a,50%a"]
|===
