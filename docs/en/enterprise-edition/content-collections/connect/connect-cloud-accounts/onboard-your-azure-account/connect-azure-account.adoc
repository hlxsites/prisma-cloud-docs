...
+
NSG flow logs, a feature of Network Watcher, allow you to view ingress and egress IP traffic information through a NSG. This step is required only if you are using the Azure Tenant or Subscription workflow, or if you would optionally like to ingest flow logs. You can also opt to ingest https://learn.microsoft.com/en-us/azure/network-watcher/vnet-flow-logs-overview?tabs=Americas[VNet flow logs] (Virtual network flow logs) instead of NSG flow logs. Follow the same steps listed below to ingest VNet flow logs.
