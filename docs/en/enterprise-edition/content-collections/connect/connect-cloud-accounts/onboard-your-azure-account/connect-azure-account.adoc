...
. Configure the networking, firewall and virtual networks settings of your Azure storage account to allow Prisma Cloud to identify internet exposed public containers. To do this Prisma Cloud requires access to the `publicContainersList` field to calculate the `totalPublicContainers` filed for azure-storage-account-list API resources. Follow the steps below to enable access:
+
.. On the Azure portal, set Public network access to *Enabled from all networks*.
.. Select *Virtual Networks* and add all the IP addresses and Prisma Cloud NAT Gateway source and DR IPs listed https://docs.prismacloud.io/en/classic/cspm-admin-guide/get-started-with-prisma-cloud/enable-access-prisma-cloud-console#idcb6d3cd4-d1bf-450a-b0ec-41c23a4d4280[here] to the firewall configuration.

[NOTE]
====
If *Public Network Access* is set to *Disabled* Prisma Cloud will not be able to ingest the `publicContainersList` field and calculate the `totalPublicContainers` filed for the Azure Storage account.
====
