...
+
*NOTE:* Entra ID authentication for Azure flowlogs is only supported for GPV2 https://learn.microsoft.com/en-us/azure/storage/common/storage-account-create?tabs=azure-portal[storage accounts]. Legacy GPV1 accounts are not supported. 
+
...
+
*NOTE:* For Flat Namespace (FNS) based storage accounts, shared key disabled works only when soft delete is disabled. For more information, reference https://learn.microsoft.com/en-us/fabric/data-factoryconnector-troubleshoot-azure-data-lake-storage#error-code-adlsgen2operationfailed[Microsoft documentation].
