...
. On the Azure portal, enable Prisma Cloud to obtain network traffic data from Virtual network (VNet) flow logs. Learn more about https://learn.microsoft.com/en-us/azure/network-watcher/vnet-flow-logs-overview?tabs=Americas[VNet flow logs]. 
+
VNet flow logs, a feature of Network Watcher, allow you to view ingress and egress IP traffic information through a Virtual Network. This step is required only if you are using the Azure Tenant or Subscription workflow, or if you would optionally like to ingest flow logs. You can also opt to ingest Network Security Group (NSG) https://docs.microsoft.com/en-us/azure/network-watcher/network-watcher-nsg-flow-logging-portal[flow logs] instead of VNet flow logs. Follow the same steps listed below to ingest NSG flow logs.
...
.. From the Network Watcher console, navigate to flow logs and complete the following steps to complete enabling VNet flowlogs:
+
... Select the Azure subscription of the VNet you want to log. 
