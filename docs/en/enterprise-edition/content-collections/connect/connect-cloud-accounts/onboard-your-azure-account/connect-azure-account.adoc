...
+
tt:[NOTE:]Prisma Cloud is unable to download flowlogs from Flat Namespace (FNS) based storage accounts that are soft delete enabled. This may lead to users encountering an Azure error, "This endpoint does not support BlobStorageEvents or SoftDelete." To resolve this issue, ensure that soft-keys are disabled. Remember that, Shared key disabled works only when soft delete is disabled but not if it is enabled. For more information reference https://learn.microsoft.com/en-us/fabric/data-factory/connector-troubleshoot-azure-data-lake-storage#error-code-adlsgen2operationfailed[Microsoft documentation].
