...
. On the Azure portal, you also have the option to setup Virtual network https://learn.microsoft.com/en-us/azure/network-watcher/vnet-flow-logs-overview?tabs=Americas[VNet flow logs] to ensure Prisma Cloud’s Service Principal has access to the storage account.
+
...
+
tt:[NOTE:] You can also opt to ingest Network Security Group (NSG) flow logs instead of VNet flow logs. Note that Microsoft will retire NSG flow logs as of 9/30/2027. As part of this sunset notice, you will no longer be able to creat NSG flow logs after 6/30/2025. We recommend migrating to VNet flow logs to avoid these restrictions.
+
