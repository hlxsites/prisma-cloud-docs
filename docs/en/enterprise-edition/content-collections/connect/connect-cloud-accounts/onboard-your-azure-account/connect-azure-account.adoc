...
* For Flat Namespace (FNS) based storage accounts, Entra ID works only when soft delete is disabled. For more information, reference https://learn.microsoft.com/en-us/fabric/data-factory/connector-troubleshoot-azure-data-lake-storage#error-code-adlsgen2operationfailed[Microsoft documentation]. +
* Entra ID authentication for Azure flowlogs is only supported for GPV2 https://learn.microsoft.com/en-us/azure/storage/common/storage-account-create?tabs=azure-portal[storage accounts]. Legacy GPV1 accounts are not supported. 
+
* Grant Storage Blob permissions at the tenant level, to ensure proper data ingestion. The UI will show that permissions are missing if permissions are granted only at the storage account/blob level and tenant-level access is not provided. 
...
.. Add the xref:../../../get-started/console-prerequisites.adoc[NAT GatewayIP addresses] for your Prisma Cloud instance to the Storage Account created in the step above. For example, if your instance is on `app.prismacloud.io` use the IP addresses associated with that.
