...
+
tt:[NOTE:] You can also opt to ingest Network Security Group (NSG) flow logs instead of VNet flow logs. Note that Microsoft will https://azure.microsoft.com/en-us/updates?id=Azure-NSG-flow-logs-Retirement[retire] NSG flow logs as of 9/30/2027. As part of this action, you will no longer be able to create NSG flow logs after 6/30/2025. We recommend https://learn.microsoft.com/en-us/azure/network-watcher/vnet-flow-logs-overview?tabs=Americas[disabling NSG flow logs] and then migrating to VNet flow logs to avoid these restrictions. Keep in mind that the option to ingest VNet flow logs is not available for Azure China tenants.
+
