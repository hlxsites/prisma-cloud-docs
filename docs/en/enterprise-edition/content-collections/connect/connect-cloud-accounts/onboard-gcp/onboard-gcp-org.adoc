...
+
Account groups that are created automatically are indicated with image:automap-icon.png, and cannot be edited on Prisma Cloud. See https://docs.paloaltonetworks.com/prisma/prisma-cloud/prisma-cloud-admin/manage-prisma-cloud-administrators/create-account-groups[create account groups] for more details.
+
...
.. Ensure that all the security capabilities you selected display a green *Enabled* (image:onboard-status-enabled.png) icon. 

.. For the security capabilities that display a red *Checks Failed* (image:onboard-status-check-failed.png) icon, click the corresponding drop-down to view the cause of failure. 
