...
+
NOTE: Since the auto-complete model is static and is not built on live data from your accounts, some of the fields may not be displayed on selecting *JSON RULE IS*. If you find few fields missing, do not use auto-complete; instead switch to the *Advanced* mode and manually enter the RQL and JSON path.
+
