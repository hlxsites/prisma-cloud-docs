...
In addition to the above attributes, you can include other optional attributes that are specific to Kubernetes:
...
You can also combine the new attributes with the existing ones:

dest.cloud.account - The cloud account/subscription/project in scope. If none is specified, ANY cloud account will be considered
protocol.ports - The specific protocols and destination port that can be specified. If none is specified, ANY port/protocol will be considered.
source/destination.vpc - The specific VPC scope. If none is specified, All VPCS will be considered
source/destination.cloud.region - Use the source/destination.cloud.region attribute to narrow down ‘TRUE network exposure’ search where the sources are in one or more cloud regions. 
effective.action  (optional)
The net effective action that ‘allow’ or ‘reject’ the network traffic from the given source to destination. Valid options are:
allowed - Routing path exists and Security Policies ALLOW traffic
denied - Routing path exists but Security Policies REJECT the traffic
Reachable - Routing path exists but Security Policies ALLOW/REJECT the traffic
