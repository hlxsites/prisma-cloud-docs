...
. Click *Add Filter* (image:add-filter-icon.PNG[]) and select menu:Policy{sp}Type[IAM].
+
...
+
Optional (mac/linux only)—Use the export command to set your environment variables.
+
...
[.procedure]
. *Add Filter* (image:add-filter-icon.PNG[]) and select menu:Policy{sp}Type[IAM] and menu:Cloud{sp}Type[GCP].
