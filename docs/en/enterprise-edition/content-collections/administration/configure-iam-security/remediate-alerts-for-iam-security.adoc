...
+
Select the alert and click *Investigate* from the Actions menu. Here you can take a closer look at the asset that trigerred the policy violation. After a policy violation is triggered, it is sent to the SQS queue. In the example below the SQS queue indicates that the alert that was triggered.
+
