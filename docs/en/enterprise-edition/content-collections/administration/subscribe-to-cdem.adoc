...
.. Verify if your *DUNS* number and the associated *Domains* are correct. If the *DUNS* and *Domain* fields are not automatically populated, you can manually enter the details.
+
CDEM uses the DUNS number and the Domains to scan for internet-exposed resources.
+
