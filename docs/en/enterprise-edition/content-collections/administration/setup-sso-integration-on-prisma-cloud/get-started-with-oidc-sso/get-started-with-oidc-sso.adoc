...
. Login to the Prisma Cloud console using the URL `https://appX<where X is the instance where your tenant is provisioned, for example app3>.prismacloud.io/login` and enter your username and password. to verify that SP initiated SSO set up is complete. 
