...
[CAUTION]
====
To avoid account lockout while making configuration changes you may enable administrative users to log into the console with both local authentication credentials on Prisma Cloud and SSO access by adding them to the SSO bypass list.
====
...
+
.. Log in to Prisma Cloud and select *Settings > Access Control > SSO > SAML*.
...
NOTE: If you have an Access URL defined use that URL for login alternatively use your xref:../access-prisma-cloud.adoc[federated login URL].
