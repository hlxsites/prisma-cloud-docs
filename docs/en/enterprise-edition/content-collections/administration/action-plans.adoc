...
[NOTE]
====
The following caveats apply to the generation of Action Plans:

- Created only for High and Critical Alerts.
- Created for "Out Of Box Policies" belonging to a risk policy category. Incidents are not considered.
- Created only if there are multiple related alerts that violate policies that having finding types. Some policies do not have finding types.
====
