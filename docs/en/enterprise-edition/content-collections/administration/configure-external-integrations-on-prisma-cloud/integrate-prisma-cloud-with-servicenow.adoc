...
. [[generate-a-token, Generate Refresh Token]] *Generate Refresh Token*
+
...
+
Once the *Refresh Token* expires, you must perform step 5—<<generate-a-token>> and include it in the Prisma Cloud UI. For example, if you set the token value as 8,640,000 seconds (100 days), then after 100 days, you must reauthorize the OAuth connection.

//Once the *Refresh Token* expires, you must perform the step 5—xref:integrate-prisma-cloud-with-servicenow.adoc#idce37e68b-d094-4b6b-a5d4-ab21d092fd36[Generate Refresh Token] and include it in the Prisma Cloud UI. For example, if you set the token value as 8,640,000 seconds (100 days), then after 100 days, you must reauthorize the OAuth connection.
