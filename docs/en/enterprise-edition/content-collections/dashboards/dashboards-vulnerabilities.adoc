...
[NOTE]
====
The dashboard is only available to users with the System Administrator role on all stacks, with the exception of http://app.gov[app.gov], http://app.cn[app.cn.]. http://app.gov[app.gov] users with a business license also do not have access to the dashboard.
====
