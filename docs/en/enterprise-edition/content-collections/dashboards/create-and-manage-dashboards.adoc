...
Note: If an alert is opened and resolved more than once a day, the latest resolution time is used for the MTTR calculation. An individual alert can be counted multiple times, if the same alert is resolved several times over multiple days. 
...
Note: If you select *Time Range > Custom*, only the Start Date day will be applied to the widget, not the hourly timestamp. In addition, the End date will always be the current date. 
