...
NOTE: Keep the following caveats in mind when sharing dashboards:
* If you are a user and you've received a URL for a shared Dashboard and you haven't updated the dashboard's *Visibility* settings, the shared Dashboard will only show up temporarily on the Menu tab. To ensure it's always accessible follow the steps listed above. 
* Users you share a dashboard with may see a `permission denied` error message if they do not have the appropriate access levels for individual widgets within the dashboards.  
