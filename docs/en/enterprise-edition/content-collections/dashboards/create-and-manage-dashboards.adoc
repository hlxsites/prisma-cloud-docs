...
[.procedure]
. Select *Dashboards* from the Prisma Cloud administrative console. Make a note of the Prisma Cloud switcher mode you are on, for instance Cloud, Runtime or Application Security. Custom dashboards created in a specific mode are only viewable in that mode. 
. Follow the steps below to configure your dashboard for public access:
...
. Follow the steps below to change the Visibility setting of your dashboard:
.. Selct *Dashboards> Menu > Manage Views*.
.. Locate your custom dashboard and toggle on the *Visibility* setting.
.. Drag your custom dashboard to the top four of the list, to make it visible to other users, when they log into your tenant and view the main Dashboards navigation menu. 
.. You can also share the URL of your dashboard with anyone in your organization to provide access.
