...
//CWP-62780
|*PCSUP-26131*
|When deploying a Fargate Defender with the Nginx server, the Nginx server was inaccessible through a browser, despite both containers being launched successfully. 
Additionally, no access logs were generated in the CloudWatch log file.

*Workaround*

* Customize the Nginx container's error logging behavior by performing the following steps.
