...
. Create a custom role under *Manage > Authentication > Roles* with write permissions for *Container Runtime Results* and *Image Analysis Sandbox*, read permissions for *CI Results*. For roles created via the API, also add write permission for the user.
