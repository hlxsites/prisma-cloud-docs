...
|A new `service` parameter has been introduced to the [Get Discovered Cloud Entities|https://pan.dev/compute/api/get-cloud-discovery-entities/] API response to specify the discovered GCP Cloud Run service name. This parameter only applies to Prisma onboarded accounts.
