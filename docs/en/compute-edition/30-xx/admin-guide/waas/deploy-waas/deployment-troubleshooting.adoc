...
. Click on the workload in the radar and open xref:./deployment-troubleshooting.adoc#connectivity_monitor[`WAAS connectivity monitor`] by clicking on the WAAS tab.
...
** __Interstitial pages served__ - Count of interstitial pages served by WAAS (interstitial pages are served once xref:../waas-advanced-settings.adoc#prisma_session[Prisma Sessions Cookies] are enabled).
...
|Click on `View recent errors` in the xref:./deployment-troubleshooting.adoc#connectivity_monitor[`WAAS connectivity monitor`] to view errors. 
...
Please see xref:../waas-advanced-settings.adoc#prisma_session[`Prisma Session Cookies`] section for more details.
...
Add xref:../waas-app-firewall.adoc#firewall_exceptions[exceptions] to protections causing false triggers.
