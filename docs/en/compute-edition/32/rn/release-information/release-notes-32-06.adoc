...
!Package Author
!Applicable only for specific package types such as rpm, jem, apk, and deb
!PURL Identifier
...
!Time of printing
!Type
!Type of asset. Images (in all stages) are categorized as containers. Serverless functions (in all stages) are categorized as services. Hosts and VMs are categorized as frameworks.
!Name
!Asset ID of images, hosts, and functions
!===
...
*ID*: asset ID of images, hosts, and functions (Obtain ID from scan results. An empty list returns all entities under the same type.)
