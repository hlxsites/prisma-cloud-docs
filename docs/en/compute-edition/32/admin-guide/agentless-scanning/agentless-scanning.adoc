...
- *Scanning* - Indicates that the scan process is in progress, which includes pre-flight checks for permissions.
    ...
    * At this state, all scanners, snapshots, and disks are cleaned up immediately.

- *Cleanup* - Indicates that if networking resources were created, they are cleaned up across all accounts and regions.

NOTE: Accounts move to this state of networking resources cleanup only after all accounts have finished scanning.
