...
If the Defenders are behind a proxy or firewall, ensure that the following URLs are whitelisted to enable seamless communication:

* *Console URL*: Ensure Defenders can connect to the Prisma Cloud Console URL. 
* *API Path*: Allow the API endpoint URL (only applicable to SaaS Consoles and not self-hosted). For example, `api.<region>.prismacloud.io`. 
+ 
Here `region` is the location of the Prisma Cloud instance. For example `sg` for Singapore or `us` for the United States. 
* *WildFire Service*: If WildFire is enabled, allow connections to `*.wildfire.paloaltonetworks.com` or the corresponding regional WildFire URLs.
