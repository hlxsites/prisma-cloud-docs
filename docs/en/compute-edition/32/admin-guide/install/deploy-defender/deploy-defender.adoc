...
* *API Path*: Defenders need to access the API endpoint URL. For example, `api.<region>.prismacloud.io`. Here `region` is the location of the Prisma Cloud instance. For example 'sg' for Singapore or 'us' for the United States. 
